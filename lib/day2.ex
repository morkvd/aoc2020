...
"""
  ...
  def pipeline(input, validator) do
    input
    |> clean_input()
    |> Stream.map(fn line ->
      case line do
        [rule, pw] -> validator.(rule, pw)
      end
    end)
    |> count_valid
  end
  ...
  def star1 do
    @input
    |> pipeline(&Day2.validate_v1/2)
  end

  def star2 do
    @input
    |> pipeline(&Day2.validate_v2/2)
  end

  def validate_v1(rule, pw) do
    ...
    case char_count do
      char_count when min <= char_count and char_count <= max ->
        {:valid, pw}
      _ ->
        {:invalid, pw}
    end
  ...
  def validate_v2(rule, pw) do
    {first, second, char} = parse_rule(rule)

    char_list = pw
    |> String.codepoints()

    char_at_first_pos = Enum.at(char_list, first - 1)

    char_at_second_pos = Enum.at(char_list, second - 1)

    case {char_at_first_pos, char_at_second_pos} do
      {a, b} when a == char and b != char ->
        {:valid, pw}
      {a, b} when a != char and b == char ->
        {:valid, pw}
      _ ->
